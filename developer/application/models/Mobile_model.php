...
class Mobile_model extends CI_Model{
	...
	public function vehicle_external_details($where){
		 ...
		 $this->db->select("vd.id,vd.name,vd.department_id,vd.description,vd.workshop_status, vg.name as group_name,vg.move_icon as run,vg.idle_icon as idle,vg.stop_icon as stop,d.name as daydrivername,d.phone as daydriverphone,d2.name as nghtdrivername,d2.phone as nghtdriverphone");
	...
	public function gps_lastrows($where){//getting full last rows
		...
		$this->db->select("vgi.`vehicle_id`,vgi.device_timestamp,vgi.lattitude,vgi.lognitude,vgi.speed,vgi.ign_status,vgi.Heading,vgi.odometer,vgi.unpluged,vgi.created,vgi.satelite,vs.status as vehicle_status,vs.start_time,vs.end_time");
