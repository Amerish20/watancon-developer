  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      <form class="cmxform" id="commentForm" method="get" action="">
       ...
       <div class="col-md-12">
      <div class="col-md-6" >
          <label>Sim Serial Number</label> <input type="number" minlength="19" maxlength="19" min="0" onkeypress="return (event.charCode == 8 || event.charCode == 0) ? null : event.charCode >= 48 && event.charCode <= 57" name="sim_serial_no" id="sim_serial_no" class="form-control"  placeholder="Serial Number" required title="19 Numbers required">
      ...
      <div class="col-md-6" >
       <label>Sim Number</label><input type="number" minlength="8" maxlength="10" min="0" onkeypress="return (event.charCode == 8 || event.charCode == 0) ? null : event.charCode >= 48 && event.charCode <= 57" name="sim_num" id="sim_num" class="form-control" placeholder="Sim Number" required title="Please Enter a Valid Mobile Number">
          <span id="name_error2" class="collapse" style="color:#F00;">Mobile Number Already existed</span>
         </div>
      </div>
       </div>
   ...
   <div class="card-body">
     <div id="table_container">    
      ...
      <table id="example" class="table table-striped table-bordered display nowrap" width="100%">       
         ...
         <input type="hidden" id="sim_num_id" value="0" />
         <input type="hidden" id="sim_serial_id" value="0" />
...
$(document).ready(function() {
	  ...
	  if (pasteData.match(/[^0-9]/))
   	...
   	$("#sim_insert").click(function(){
  	  ...
  	  if($("#commentForm").valid()){
  		 $("#sim_insert").attr("disabled", "disabled");
		 ...
		 //var status=$("#status").val();
			     ...
			     success: function(result){
					$("#sim_insert").removeAttr("disabled");
 ...
 $(document).on("input", '#sim_serial_no', function (event) {
		 ...
		 success: function(result){
			 ...
			 if(data>0){
				  ...
				  $("#name_error1").addClass("collapse");
				   ...
				   $("#sim_serial_id").val(0);
				   var sim_serial_id=$("#sim_serial_id").val();
				    var sim_num_id=$("#sim_num_id").val();
				   if(sim_serial_id=="0" && sim_num_id=="0"){
				  		$("#sim_insert").removeAttr("disabled");
				   }
			 }
 		 }
 ...
 $(document).on("input", '#sim_num', function (event) {
 	 var sim_no=$(this).val();
	 $.ajax({
		 url: "<?=base_url()?>Sim/sim_num_check", 
		 data:{sim_number:sim_no},
		 method: "POST",
		 success: function(result){
 			 var data = $.parseJSON(result); 
			 ...
			 if(data>0){
				  ...
				  $("#name_error2").addClass("collapse");
				   ...
				   var sim_num_id=$("#sim_num_id").val();
				    var sim_serial_id=$("#sim_serial_id").val();
				   if(sim_num_id=="0" && sim_serial_id=="0"){
				  		$("#sim_insert").removeAttr("disabled");
				   }
	 ...
	 $(document).on("click", '#sim_update', function (event) {
  		  ...
  		  if($("#commentForm").valid()){
			  $("#sim_update").attr("disabled", "disabled");
			 swal({
                      title: "Are you sure?",
			    ...
			    $.ajax({
			      ...
			      method: "POST",
			      success: function(result){
					$("#sim_update").removeAttr("disabled"); 
  					 var data = $.parseJSON(result); 
					 if(data.error_flag=="1"){
						 swal({
                                 text: data.error_message,
